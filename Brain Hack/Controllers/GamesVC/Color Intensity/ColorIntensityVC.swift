...
class ColorIntensityVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        btnCollection.register(UINib(nibName: "colorCell", bundle: nil), forCellWithReuseIdentifier: "colorCell")
        self.btnCollection.reloadData()
        self.btnCollection.delegate = self
        self.btnCollection.dataSource = self
        getRandomColor()
    }
    ...
    @IBAction func btnTap_pause(_ sender: UIButton) {
        if is_PAUSE {
            self.btn_pause.setImage(UIImage(named: "ic_pause"), for: .normal)
            timer = Timer.scheduledTimer(timeInterval: 0.01, target: self, selector: #selector(updateTimer), userInfo: nil, repeats: true)
            is_PAUSE = false
        }
        else {
            self.btn_pause.setImage(UIImage(named: "ic_play"), for: .normal)
            timer.invalidate()
            is_PAUSE = true
        }
    }
...
extension ColorIntensityVC : UICollectionViewDelegate,UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.lable.backgroundColor = arrDisplayColorUI[indexPath.row] as? UIColor
        return cell
    }
...
extension ColorIntensityVC : UICollectionViewDelegateFlowLayout {
