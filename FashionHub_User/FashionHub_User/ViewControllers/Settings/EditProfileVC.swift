...
class EditProfileVC: UIViewController {
  ...
  override func viewDidLoad() {
    super.viewDidLoad()
    self.txt_name.placeHolderColor = .lightGray
    self.txt_email.placeHolderColor = .lightGray
    self.txt_mobileNumber.placeHolderColor = .lightGray
  ...
  @IBAction func btnTap_Update(_ sender: UIButton) {
    ...
    if self.txt_mobileNumber.text! == "" {
      ...
      let urlString = "http://192.168.1.28/ecom-saas/api/editprofile"
    ...
    WebServices().multipartWebService(method:.post, URLString:urlString, encoding:JSONEncoding.default, parameters:params, fileData:imageData!, fileUrl:nil, headers:headers, keyName:"image") { (response, error,statusCode)  in
      ...
      if error != nil {
        ...
        if statusCode == "1" {
          ...
          self.img_profileImage.sd_setImage(with: URL(string: "http://192.168.1.28/ecom-saas/api/editprofile" + "\(data["message"] as! String)"), placeholderImage: UIImage(named: "ic_placeholder"))
          UserDefaultManager.setStringToUserDefaults(value: "\(data["image"] as! String)", key: UD_Userprofile)
