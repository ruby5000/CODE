...
class UserDefaultManager
{
  ...
  class func setCustomObjToUserDefaults(CustomeObj: [String:String] , key: String) {
    let defaults = UserDefaults.standard
  ...
  class func setCustomObjToUserDefaultsGuest(CustomeObj: [[String:String]] , key: String) {
    let defaults = UserDefaults.standard
    let encodedData = NSKeyedArchiver.archivedData(withRootObject: CustomeObj)
    defaults.set(encodedData, forKey: key)
    defaults.synchronize()
  ...
  class func getCustomObjFromUserDefaultsGuest(key: String) -> AnyObject {
    let defaults = UserDefaults.standard
    let decoded  = defaults.object(forKey: key) as! NSData
    let decodedTeams = NSKeyedUnarchiver.unarchiveObject(with: decoded as Data)! as! [[String:String]]
